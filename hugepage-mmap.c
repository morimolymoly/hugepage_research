...
void call_ioctl(unsigned long addr)
{
	int fd;
	struct value valueset;
	valueset.addr = addr;
	valueset.size = LENGTH;

	if ((fd = open("/dev/mydevice0", O_RDWR)) < 0) perror("open");

	if(ioctl(fd, HUGE_SET_VALUE, &valueset) < 0) perror("ioctl_set");
